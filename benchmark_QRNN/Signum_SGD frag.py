...
class SGD_distribute(Optimizer):
    ...
    def __init__(self, params, lr=0.01, momentum=0.9, weight_decay = 0, compression_buffer = False, all_reduce = False ,local_rank = 0, gpus_per_machine = 1 , single_worker = False, **kwargs):
        ...
        self.MB = 1024 * 1024
        self.bucket_size = 20000 * self.MB

        if self.compression_buffer and not self.single_worker:

            self.compressor = compressor.compressor(using_cuda = True, local_rank = 0)
            self.compressor = self.compressor.cuda()
            self.compressor = torch.nn.DataParallel(self.compressor,device_ids = [0,1,2,3,4,5,6,7],dim=1)
            ...
            compression_devices = ['cuda:0','cuda:1']
            self.compression_devices_iter = iter(compression_devices)
    ...
    def get_time_result(self):
        if dist.get_rank() == 0:
            ...
            self.compressor.module.get_time_result()
    ...
    def reset_time(self):
        ...
        self.compressor.module.reset_time()
        ...
        for group in self.param_groups:
            ...
            if not self.single_worker:
                ...
                for dev_grads in dev_grads_buckets:
                    ...
                    #recieved_tensor_list = torch.cuda.comm.broadcast(d_p_new,[0,1])

                    #d_p_new = recieved_tensor_list[1]
                    #print(d_p_new)
                    ...
                    if self.all_reduce:
                        ...
                        if self.nodes > 1:
                            ...
                            if self.compression_buffer:
                                d_p_new, tensor_size = self.compressor.module.compress(d_p_new)
                            else:
                                d_p_new = torch.sign(d_p_new)
                            ...
                            if self.local_rank == 0:
                                ...
                                if dist.get_rank() == 0:
                                    ...
                                    if self.compression_buffer:
                                        ...
                                        d_p_new_list_tensor = torch.stack(d_p_new_list)
                                        #print('full_size',d_p_new_list_tensor.size(),d_p_new_list_tensor)

                                        d_p_new = self.compressor.forward(d_p_new_list_tensor)
                                        d_p_new = d_p_new[0]    
                                    else:
                            ...
                            ##torch.cuda.synchronize()
                            if self.compression_buffer:
                                d_p_new = self.compressor.module.uncompress(d_p_new, tensor_size)
