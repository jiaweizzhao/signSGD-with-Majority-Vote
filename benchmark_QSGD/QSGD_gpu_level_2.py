...
def encode(v, enable_max, **kwargs):
    ...
    if v.is_cuda:
        mask_upper = mask_upper.cuda()
    selected_upper = torch.masked_select(idx, mask_upper).long()
    signs_upper = torch.Tensor(w.size()).float().type_as(w) #change int to float
    ...
    if v.is_cuda:
        mask_lower = mask_lower.cuda()
    selected_lower = torch.masked_select(idx, mask_lower).long()
    signs_lower = torch.Tensor(w.size()).float().type_as(w) #change int to float
